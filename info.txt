...
# Bot information
API_ID = environ.get('API_ID', "1383845")
if len(API_ID) == 0:
    print('Error - API_ID is missing, exiting now')
    exit()
else:
    API_ID = int(API_ID)
API_HASH = environ.get('API_HASH', '0e3d2c299cc3c5cc26c283cecd2eb97c')
if len(API_HASH) == 0:
    print('Error - API_HASH is missing, exiting now')
    exit()
BOT_TOKEN = environ.get('BOT_TOKEN', '5115970591:AAGfQTYeXIFu9ezFkhG5bGGr7bzJbLa_VKw')
if len(BOT_TOKEN) == 0:
...
# Channels
INDEX_CHANNELS = [int(index_channels) if index_channels.startswith("-") else index_channels for index_channels in environ.get('INDEX_CHANNELS', '-1001796388930').split()]
if len(INDEX_CHANNELS) == 0:
    print('Info - INDEX_CHANNELS is empty')
AUTH_CHANNEL = [int(auth_channels) for auth_channels in environ.get('AUTH_CHANNEL', '-1001876738264').split()]
if len(AUTH_CHANNEL) == 0:
    print('Info - AUTH_CHANNEL is empty')
LOG_CHANNEL = environ.get('LOG_CHANNEL', '-1001876738264')
...
# for chatGPT
OPENAI_API = environ.get('OPENAI_API', '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')
if len(OPENAI_API) == 0:
    print('Info - OPENAI_API is empty')

# MongoDB information
DATABASE_URL = environ.get('DATABASE_URL', "mongodb+srv://kmac:kmac@cluster0.pkfykvz.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
